...
const HeroSection = () => {
  ...
  return (
    <section className="pt-32 pb-20 md:pt-40 md:pb-32 bg-hero-pattern relative overflow-hidden">
      <div className="container-wide relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className={`${isLoaded ? 'opacity-100' : 'opacity-0'} transition-opacity duration-700`}>
            <Badge className="mb-4">Nieuw platform</Badge>
            <h1 className="font-bold text-balance mb-6 text-gray-900">
              Verzamel en beheer <span className="text-brand-600">klantreviews</span> op één plek
            </h1>
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-600">
              ...
              <span>Meer dan 500+ tevreden klanten</span>
            </div>
          </div>
          ...
          <div className={`${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} transition-all duration-1000 delay-300 relative`}>
            <div className="relative bg-white p-3 rounded-2xl shadow-card animate-float">
              <div className="absolute -top-12 -right-8 bg-brand-50 rounded-full h-40 w-40 z-0"></div>
              <div className="absolute -bottom-10 -left-10 bg-blue-50 rounded-full h-32 w-32 z-0"></div>
      ...
      {/* Background elements */}
      <div className="absolute top-1/3 right-[5%] w-64 h-64 bg-brand-100 rounded-full filter blur-3xl opacity-60 animate-pulse-soft"></div>
      <div className="absolute bottom-1/4 left-[10%] w-48 h-48 bg-blue-100 rounded-full filter blur-3xl opacity-60 animate-pulse-soft"></div>
    </section>
  );
};
